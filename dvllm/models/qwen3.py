...
from dvllm.layers.linear import (
    QKVParallelLinear,
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        q, k, v = qkv.split(
            [2048, 1024, 1024], dim=-1
        )  # q:[B,L,2048], k:[B,L,1024], v:[B,L,1024]
        ...
        k = k.transpose(
            1, 2
        )  # [B, L, num_kv_heads, head_dim] -> [B, num_kv_heads, L, head_dim]
        v = v.transpose(
            1, 2
        )  # [B, L, num_kv_heads, head_dim] -> [B, num_kv_heads, L, head_dim]
        ...
        if device == "mps" and L > chunked_threshold:
            ...
            for s in range(0, L, chunk_size):
                ...
                if factor > 1:
                    k_chunk = k_slice.repeat_interleave(
                        factor, dim=1
                    )  # Repeat on heads dimension
                    v_chunk = v_slice.repeat_interleave(factor, dim=1)
                else:
                    k_chunk, v_chunk = k_slice, v_slice
                out_chunk = sdpa(
                    q_chunk, k_chunk, v_chunk
                )  # [B, num_heads, chunk_size, head_dim]
